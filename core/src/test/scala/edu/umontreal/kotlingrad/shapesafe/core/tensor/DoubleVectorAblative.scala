package edu.umontreal.kotlingrad.shapesafe.core.tensor

import edu.umontreal.kotlingrad.shapesafe.BaseSpec
import edu.umontreal.kotlingrad.shapesafe.m.arity.Arity
import graph.commons.util.viz.VizType
import shapeless.Witness
import singleton.ops.+

class DoubleVectorAblative extends BaseSpec {

  val a = Arity(6)
//  val aa: Arity.FromLiteral[Witness.`6`.T] = a.out
  val aa = a.out

  val v0 = DoubleVector.random(6)
  val va = v0.shape
//  val vaa: Arity.FromLiteral[Witness.`6`.T] = va.out
  val vaa = va.out

  val aViz = VizType[a.type]

  def assert_aViz_==(v: VizType): Unit = {

    val Seq(s1, s2) = Seq(aViz, v).map { v =>
      v.toString.split("\n").slice(1, Int.MaxValue).mkString("\n")
    }

    s1.shouldBe(s2)
  }

  it("stripped to the core") {

    {
      assert_aViz_==(VizType[a.type])

      implicitly[a.SS + Witness.`3`.T]
    }

    {
      assert_aViz_==(VizType[aa.type])

      implicitly[aa.SS + Witness.`3`.T]
    }

    {
      assert_aViz_==(VizType[va.type])

      implicitly[va.SS + Witness.`3`.T]
    }

//    {
//      val aa = v0.shape
//
//      print_@(VizType[aa.Out])
//
//      implicitly[aa.SS + Witness.`3`.T]
//    }

    {
      // this doesn't

      assert_aViz_==(VizType[vaa.type])

      implicitly[vaa.SS + Witness.`3`.T]
    }
  }
}
