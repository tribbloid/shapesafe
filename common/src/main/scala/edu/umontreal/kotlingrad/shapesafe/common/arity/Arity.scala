...
object Arity {
  ...
  trait Constant[S] extends Arity {

    type Single = S // compile-type, MUST be a valid operand of singleton_ops
    def number: Int
    ...
    @transient protected[shapesafe] object internal {
      ...
      def proveSame[N2](implicit self: Single =:= N2): Unit = {}

      // 'prove' means happening only in compile-time
      def proveEqual[N2](implicit self: Require[Single == N2]): Unit = {

        //TODO: need run-time proof

      }

      def requireEqual(w: Lt[Int])(implicit self: Require[Single == w.T]): Unit = {
  ...
  class FromSize[S <: ToInt[_]](val number: Int) extends Constant[S]

  object FromSize {

    implicit def summon[S <: ToInt[_]](implicit s: S): FromSize[S] = new FromSize[S](s.value.asInstanceOf[Int])
  }

  class FromOp[S <: Op](val number: Int) extends Constant[S]
  ...
  class FromLiteral[S <: Int](val number: Int) extends Constant[ToInt[S]]
